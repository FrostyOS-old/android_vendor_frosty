...
function setup_vendor() {
    ...
    export FROSTY_ROOT="$3"
    if [ ! -d "$FROSTY_ROOT" ]; then
        echo "\$FROSTY_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$FROSTY_ROOT/$OUTDIR" ]; then
        mkdir -p "$FROSTY_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$FROSTY_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDMK="$FROSTY_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$FROSTY_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$FROSTY_ROOT"/vendor/frosty/build/tools/smali/baksmali.jar
        export SMALIJAR="$FROSTY_ROOT"/vendor/frosty/build/tools/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$FROSTY_ROOT"/vendor/frosty/build/tools/"$HOST"/vdexExtractor
    fi
    ...
    if [ ! -f "$FROSTY_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$FROSTY_TARGET" >/dev/null; then
...
function extract() {
    ...
    local SRC="$2"
    local OUTPUT_ROOT="$FROSTY_ROOT"/"$OUTDIR"/proprietary
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$FROSTY_ROOT"/vendor/frosty/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$FROSTY_ROOT"/"$OUTDIR"/radio
