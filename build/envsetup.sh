...
function breakfast()
{
    ...
    for f in `/bin/ls vendor/frosty/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the Frosty model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch frosty_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/FrostyOS-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
...
function makerecipe() {
    ...
    then
        pwd
        frostyremote
        git push frosty HEAD:refs/heads/'$1'
    fi
    '
}
...
if [ -d $(gettop)/prebuilts/snapdragon-llvm/toolchains ]; then
    case `uname -s` in
        ...
        *)
            ...
            export SDCLANG_LTO_DEFS=$(gettop)/vendor/frosty/build/core/sdllvm-lto-defs.mk
            ;;
    esac
fi
